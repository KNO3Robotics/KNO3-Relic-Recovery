...
public class CalibrateGlyphClampBR extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            double autoSetpoint = settings.getDouble("glyph_clamp_bottom_right_auto");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
                    autoSetpoint += gamepad1.left_stick_x * Math.abs(gamepad1.left_stick_x) / 3000;
                }
                if(Math.abs(gamepad1.right_stick_x) > 0.05) {
                    autoSetpoint += gamepad1.right_stick_x * Math.abs(gamepad1.right_stick_x) / 15000;
                }
                bottomRightGlyphClamp.setPosition(autoSetpoint);
                telemetry.addData("Auto Clamped Setpoint: ", autoSetpoint);
                telemetry.addData("2", "Press A to continue");
                telemetry.update();
            }
            ...
            settings.setDouble("glyph_clamp_bottom_right_auto", autoSetpoint);
            settings.save();
