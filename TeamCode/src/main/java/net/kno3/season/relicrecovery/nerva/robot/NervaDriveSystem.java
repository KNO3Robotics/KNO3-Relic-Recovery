...
public class NervaDriveSystem extends SubSystem {
    ...
    public void init() {
        ...
        Threading.startThread(() -> {
            try {
                //IAdafruitIMU exIMU = new SafeAdafruitIMU(hardwareMap(), Nerva.IMU_KEY, true);
                Log.i("IMUt", "starting imu stuff");
                IAdafruitIMU backIMU = new SafeAdafruitIMU(hardwareMap(), Nerva.IMU_1_KEY, true);
                Log.i("IMUt", "back imu initialized");
                IAdafruitIMU sideIMU = new SafeAdafruitIMU(hardwareMap(), Nerva.IMU_2_KEY, true);
                Log.i("IMUt", "side imu initialized");

                //imu = new MultiIMU(exIMU, backIMU, sideIMU);
                imu = new MultiIMU(backIMU, sideIMU);
                Log.i("IMUt", "multi imu setup");
            ...
            /*AtomicBoolean contin = new AtomicBoolean(true);

            FtcOpModeRegister.opModeManager.registerListener(new OpModeManagerNotifier.Notifications() {
                ...
                public void onOpModePostStop(OpMode opMode) {
                    contin.set(false);
                    FtcOpModeRegister.opModeManager.unregisterListener(this);
                }
            });*/
    ...
    public void driveWithCorrectionSlow(double maxSpeed, double inches, double angle, double delay, boolean pass, Supplier<Boolean> additionalStopParameter) {
        driveWithCorrection(maxSpeed, kDriveWithCorrectionsSlow_p, kDriveWithCorrectionsSlow_i, kDriveWithCorrectionsSlow_d, inches, kDriveWithCorrectionsSlow_kp, kDriveWithCorrectionsSlow_ki, kDriveWithCorrectionsSlow_kd, kDriveWithCorrectionsSlow_maxTurn, angle, kDriveWithCorrectionsSlow_tolerance, delay, pass, additionalStopParameter);
    }
    ...
    public void driveWithCorrectionFastX(double inches, double angle) {
        driveWithCorrectionFastX(inches, angle, 0.05, null);
    }

    public void driveWithCorrectionFastX(double inches, double angle, double delay, Supplier<Boolean> additionalStopParameter) {
        driveWithCorrectionX(kDriveWithCorrectionsFastX_maxSpeed, kDriveWithCorrectionsFastX_p, kDriveWithCorrectionsFastX_i, kDriveWithCorrectionsFastX_d, inches, kDriveWithCorrectionsFastX_kp, kDriveWithCorrectionsFastX_ki, kDriveWithCorrectionsFast_kd, kDriveWithCorrectionsFastX_maxTurn,angle, kDriveWithCorrectionsFastX_tolerance, delay, false, additionalStopParameter );
    }
    ...
    public void driveFor45X(double maxSpeed, double p, double inches, boolean pass) { // made some changes not sure wheter works or not. PLEASE TEST
        resetEncX();
        resetEncY();
        int targetPulses = inchesToEncoderPulses(inches);
        SynchronousPID pid = new SynchronousPID(p, 0, 0);
        ...
        Threading.waitFor(() -> {
            int encoderPos = (int)((double)getEncY() * Math.sqrt(2));
            double speed = pid.calculate(encoderPos);

            driveFieldOriented(0, -speed, 0);
    ...
    public void driveFor45X(double maxSpeed, double inches) {
        driveFor45X(maxSpeed, .0003, inches, false);
    }

    public void driveFor45X(double inches) {
        driveFor45X(0.7, inches);
    }

    public void driveFor45Y(double maxSpeed, double p, double inches, boolean pass) { // made some changes not sure wheter works or not. PLEASE TEST
        resetEncX();
        resetEncY();
        int targetPulses = inchesToEncoderPulses(inches);
        SynchronousPID pid = new SynchronousPID(p, 0, 0);
    ...
    public void driveFor45Y(double inches) {
        driveFor45Y(.7, .0003, inches, false);
    }
