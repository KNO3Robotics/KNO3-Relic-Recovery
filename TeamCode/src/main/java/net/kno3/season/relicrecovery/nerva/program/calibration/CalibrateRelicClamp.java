...
public class CalibrateRelicClamp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            double dropSetpoint = settings.getDouble("relic_clamp_drop");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
                    dropSetpoint += gamepad1.left_stick_x * Math.abs(gamepad1.left_stick_x) / 3000;
                }
                if(Math.abs(gamepad1.right_stick_x) > 0.05) {
                    dropSetpoint += gamepad1.right_stick_x * Math.abs(gamepad1.right_stick_x) / 15000;
                }
                relicClamp.setPosition(dropSetpoint);
                telemetry.addData("Drop Setpoint: ", dropSetpoint);
                telemetry.addData("2", "Press A to continue");
                telemetry.update();
            }
            ...
            double clutchSetpoint = settings.getDouble("relic_clamp_clutch");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
                    clutchSetpoint += gamepad1.left_stick_x * Math.abs(gamepad1.left_stick_x) / 3000;
                }
                if(Math.abs(gamepad1.right_stick_x) > 0.05) {
                    clutchSetpoint += gamepad1.right_stick_x * Math.abs(gamepad1.right_stick_x) / 15000;
                }
                relicClamp.setPosition(clutchSetpoint);
                telemetry.addData("clutcHH Setpoint: ", clutchSetpoint);
                telemetry.addData("2", "Press A to continue");
                telemetry.update();
            }
            ...
            settings.setDouble("relic_clamp_drop", dropSetpoint);
            settings.setDouble("relic_clamp_clutch", clutchSetpoint);
            settings.save();
